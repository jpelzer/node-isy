...
module.exports = function(ISY) {
  ...
  var doCommand = function(self, name, value, cb) {
    var cmd = "/rest/nodes/" + name + "/cmd/" + value;
    self.request(cmd, function(err, data) {
      if(err)
        return cb(err);
      cb("Success!");
    });
  ...
  var runProgram = function(self, programId, runIfThenElse, cb) {
    var cmd = "/rest/programs/" + programId + "/" + runIfThenElse;
    self.request(cmd, function(err, data) {
      if(err)
        return cb(err);
      cb("Success!");
  ...
  ISY.prototype.runIf = function(programId, cb){
    runProgram(this, programId, 'runIf',cb);
  };

  ISY.prototype.runThen = function(programId, cb){
    runProgram(this, programId, 'runThen',cb);
  };

  ISY.prototype.runElse = function(programId, cb){
    runProgram(this, programId, 'runElse',cb);
  };

  ISY.prototype.turnOn = function(name, cb) {
    doCommand(this, name, "DON", cb);
  };

  ISY.prototype.turnOff = function(name, cb) {
    doCommand(this, name, 'DOF', cb);
  };
